  ...
  if (submitted) {
    return (
      <div className="flex flex-col items-start justify-center min-h-[500px] gap-5">
        ...
        <span className="font-mono text-brand text-xs tracking-[0.3em] uppercase">Message Received</span>
      ...
      <form className="space-y-5" onSubmit={handleSubmit}>
        ...
        <div>
          ...
          <div className="relative">
            <select defaultValue="" className="w-full appearance-none bg-cream/5 border border-cream/10 text-cream/60 font-sans text-sm px-4 py-3 focus:outline-none focus:border-brand transition-colors duration-200 cursor-pointer">
              <option value="" disabled>Select a service…</option>
              {violationTypes.map((v) => (
                <option key={v} value={v} className="bg-charcoal text-cream">{v}</option>
              ))}
            </select>
            <span className="pointer-events-none absolute right-4 top-1/2 -translate-y-1/2 text-brand text-xs">▼</span>
          </div>
        </div>
        <div>
